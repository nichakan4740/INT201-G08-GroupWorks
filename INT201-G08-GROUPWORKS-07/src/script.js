...
function showProducts(products) {
  ...
  products.forEach((product, index) => {
    ...
    productList.appendChild(productItem); //เพิ่ม productItem เข้าไปใน div ของ productList ซึ่งตรงกับไอดี product-container ที่เราดึงมา
...
function searchProduct(searchName) {
  ...
  const filteredProduct = products.filter(
    ...
    ) => product.productName.toLowerCase().includes(searchName.toLowerCase()) // เอาเฉพาะ product ที่ชื่อตรงกับที่ User search โดยได้เปลี่ยนเป็นตัวพิมพ์เล็กในเริ่มแรก
  );
...
function removeAllProducts() {
  const elements = document.getElementsByClassName("productItem"); //เชื่อมไปยัง node ที่มี class productItem ดึงทุกnode ที่ชื่อ productItem
